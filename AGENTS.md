...
| [`ts/test/`](ts/test/) | TS tests (`.ts`, compiled to `dist-test/`): `zon.test.ts` (parse cases), `debug-model.test.ts` (the `@tabnas/debug` composition / model introspection), `doc-examples.test.ts` (runs `// =>` assertions in README/doc fences), `zigzon.test.ts` + `strictness.test.ts` (conformance — see below). |
| [`scripts/fetch-zigzon.sh`](scripts/fetch-zigzon.sh) | Fetches the pinned ziglang/zig 0.16.0 release and **generates** the conformance corpora. The corpora are `.gitignore`d and **must never be committed**. |
| [`test/zigzon/`](test/zigzon/README.md) | Conformance corpus from the Zig reference implementation. `tools/` is tracked; `cases.json` and `vendor/` are generated. |
| [`test/strictness/`](test/strictness/README.md) | Leniency probe. `inputs.txt` is ours and tracked; `cases.json` is generated (verdicts from the same Zig oracle). |
...
## Conformance suites (currently RED — on purpose)

Two suites measure this plugin against the **Zig reference implementation**
rather than against itself. Both auto-run under `npm test` / `go test ./...`,
both cover the valid *and* the invalid half, and both compare **values**, not
merely "it didn't throw".

| Suite | Corpus | TS runner | Go runner |
|---|---|---|---|
| `zigzon` | [`test/zigzon/`](test/zigzon/README.md) — 222 documents (178 valid / 44 invalid) extracted from ziglang/zig `0.16.0` | `ts/test/zigzon.test.ts` | `go/zigzon_test.go` |
| `strictness` | [`test/strictness/`](test/strictness/README.md) — 117 locally authored inputs (45 valid / 72 invalid), verdicts from the same oracle | `ts/test/strictness.test.ts` | `go/strictness_test.go` |

**The corpora are FETCHED, never committed.** `bash scripts/fetch-zigzon.sh`
(idempotent) downloads the pinned zig 0.16.0 release — verified by SHA-256 —
and generates both `cases.json` files into `.gitignore`d paths. Committing a
third-party corpus is a hard project rule violation; do not do it.

Verdicts and expected values come from `test/zigzon/tools/oracle.zig`, which
runs `std.zig.Ast` in `.zon` mode plus `std.zig.ZonGen` at that pinned version.
`ZonGen` is the right layer: it lowers ZON to an **untyped** value tree, exactly
what this plugin produces. The layer above (`std.zon.parse`) also type-checks
against a Zig type, and those are type errors, not ZON errors.

**TRUE BASELINE, observed 2026-08-07** (no parser change — `src/zon.ts` and
`go/zon.go` are byte-identical to `main`):

```
zigzon      TS  valid-accepted 148/178 (83.1%)   invalid-rejected 37/44 (84.1%)
            Go  valid-accepted 148/178 (83.1%)   invalid-rejected 37/44 (84.1%)
strictness  TS  valid-accepted  37/45  (82.2%)   invalid-rejected 52/72 (72.2%)
            Go  valid-accepted  37/45  (82.2%)   invalid-rejected 56/72 (77.8%)

npm test:  415 tests, 350 pass, 65 FAIL, 0 skipped
go test:   347 pass, 67 FAIL, 0 skipped
```

**These are measuring instruments, not gates.** Do not shrink a corpus, add a
skip list, narrow the option set, or loosen the value comparison to make a
number look better — a conformance figure that cannot fail is worth nothing.
Fix the grammar, or leave the case red with a written reason.

**They must never skip.** If a corpus is absent the suites FAIL with
instructions. `ts/package.json` has a `pretest` hook that runs the fetch, the
`Makefile` `test-go` target depends on a `corpus` target, and
`.github/workflows/conformance.yml` runs the fetch as an explicit step before
the tests. A conformance test that quietly does not run is worse than no test.

### The dominant cause: inherited jsonic leniency

This is a **jsonic plugin and cannot run without jsonic** —
`new Tabnas().use(Zon)` alone cannot parse even `.{ .a = 1 }`, or anything
else. So unlike the json5 plugin (where the standalone plugin is stricter than
the documented stack), there is **no stricter mode to fall back to**: jsonic's
relaxed number lexer and its "first value wins, ignore the rest" top-level
behaviour are inherited unconditionally by the only supported setup.

The documented stack accepts, and Zig rejects: `+1`, `-0`, `.5`, `5.`, `0123`,
`00`, `1__0`, `0x_2A`, `.0`, `.{ .a = 1, .a = 2 }` (duplicate field), the
field-access family (`.a.b`, `.{}.a`, `.{ .a = 1 }.a`, `.foo .bar`), and the
trailing-content family (`1 2` → 1, `true false` → true, `"a" "b"` → "a",
`.{ .a = 1 },` → {a:1}).

Note that `ts/doc/reference.md:284` already promises "Inputs that are valid
jsonic but not valid ZON ... are errors". The probe shows that promise is not
currently kept.

### TS/Go divergence (TS is canonical)

20 inputs are classified differently by the two runtimes. Two families:

- **TS accepts, Go rejects** — trailing content / field access at the top
  level: `1 2`, `true false`, `"a" "b"`, `.foo .bar`, `.a.b`, `.{}.a`,
  `.{ .a = 1 }.a`, `.{ .a = 1 },`. Zig rejects all of them, so Go is
  accidentally right and **TS is the side that must change**.
- **Go accepts, TS rejects** — uppercase base prefixes and a trailing
  separator: `0X2A`, `0O52`, `0B1010`, `1_`. Zig rejects all four, so **Go is
  the side that must change**. (Go's `number` block carries extra
  `Hex`/`Oct`/`Bin`/`Sep` settings the TS side leaves to jsonic defaults — see
  alignment rule 4.)
- Plus 8 corpus documents with integers beyond f64 (`0x1ffffffffffffffff`,
  `0b1…`, `0o3777…`): TS accepts them lossily as doubles, Go rejects them.
  Both are wrong — Zig keeps them exact — but they diverge.
